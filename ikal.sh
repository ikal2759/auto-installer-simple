...
check_token() {
  ...
  if [ "$USER_TOKEN" = "kalstoredev" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di Skyzopedia${NC}"
    echo -e "${YELLOW}TELEGRAM : @ikal2749${NC}"
    echo -e "${YELLOW}WHATSAPP : +6283839153727${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© KalStore${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  ...
  THEME_URL="https://github.com/ikal2759/Auto-Installer-Simple/raw/main/${THEME_NAME}.zip"
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer KalStore   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : @ikal2749      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : ©KalStore  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by KalStore  ${NC}"
